...
def eval_psnr(loader, model, data_norm=None, eval_type=None, eval_bsize=None, verbose=False):
    ...
    for batch in pbar:
        ...
        if eval_type is not None:
            if not eval_type.startswith('ssim'):  # reshape for shaving-eval
                ...
                for idx in range(pred.shape[0]):
                    p_img = pred[idx].permute(1, 2, 0).cpu().numpy()
                    g_img = batch['gt'][idx].permute(1, 2, 0).cpu().numpy()
                    multichannel_flag = True
                    if p_img.shape[-1] == 1:
                        p_img = p_img.squeeze()
                        g_img = g_img.squeeze()
                        multichannel_flag = False
                    score = metric_fn(p_img, g_img, multichannel=multichannel_flag)
                    val_res.add(score, inp.shape[0])
        else:
